...
function LocationPreviewMap({ la, lo }: { la: number; lo: number }) {
  ...
  return (
    <div className="mt-4 overflow-hidden rounded-xl border border-[var(--border)] bg-[var(--bg-base)]">
      <p className="border-b border-[var(--border)] bg-[var(--bg-surface)] px-3 py-2 text-xs font-medium text-[var(--text-secondary)]">
...
export const ClientForm: React.FC<ClientFormProps> = ({ onAddClient }) => {
  ...
  return (
    <div className="max-w-3xl mx-auto bg-[var(--bg-surface)] p-8 rounded-xl shadow-[var(--shadow-md)] border border-[var(--border)]">
      <div className="flex items-center mb-6">
        <User className="w-6 h-6 text-blue-900 mr-2" />
        <h2 className="text-2xl font-bold text-[var(--text-primary)]">Alta de Nuevo Cliente</h2>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="md:col-span-2">
            ...
            <input
              type="text"
              required
              className="w-full border-[var(--border)] rounded-lg p-2.5 bg-[var(--bg-base)] focus:ring-2 focus:ring-blue-500 outline-none border transition-shadow"
        ...
        <div>
          <label className="block text-sm font-medium text-[var(--text-secondary)] mb-1 flex items-center">
            <Hash className="w-3 h-3 mr-1 text-[var(--text-muted)]" /> RUT (12 Dígitos)
          </label>
          <input
            type="text"
            required
            placeholder="219999990012"
            className="w-full md:w-1/2 border-[var(--border)] rounded-lg p-2.5 bg-[var(--bg-base)] outline-none border font-mono"
            value={formData.rut || ''}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <label className="block text-sm font-medium text-[var(--text-secondary)] mb-1 flex items-center">
              <Mail className="w-3 h-3 mr-1 text-[var(--text-muted)]" /> Email
            </label>
            <input
              type="email"
              required
              placeholder="contacto@empresa.com"
              className="w-full border-[var(--border)] rounded-lg p-2.5 bg-[var(--bg-base)] outline-none border"
          ...
          <div>
            <label className="block text-sm font-medium text-[var(--text-secondary)] mb-1 flex items-center">
              <Phone className="w-3 h-3 mr-1 text-[var(--text-muted)]" /> Teléfono
            </label>
            <input
              type="tel"
              required
        ...
        <div className="border-t border-[var(--border-subtle)] pt-4">
          <h3 className="text-sm font-semibold text-[var(--text-muted)] uppercase tracking-wider mb-3 flex items-center">
            <MapPin className="w-4 h-4 mr-1" /> Ubicación
          </h3>
          <div className="mb-4 flex gap-2 rounded-lg border border-[color-mix(in_srgb,var(--accent-blue)_30%,transparent)] bg-[var(--accent-blue-muted)] p-3 text-xs text-[var(--text-secondary)]">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-[var(--text-secondary)] mb-1">Departamento</label>
              <select
                className="w-full border-[var(--border)] rounded-lg p-2.5 bg-[var(--bg-base)] outline-none border"
            ...
            <div>
              <label className="block text-sm font-medium text-[var(--text-secondary)] mb-1">Localidad</label>
              <input
                type="text"
                required
                className="w-full border-[var(--border)] rounded-lg p-2.5 bg-[var(--bg-base)] outline-none border"
            ...
            <div>
              <label className="block text-sm font-medium text-[var(--text-secondary)] mb-1">Latitud</label>
              <input
                type="number"
                step="any"
                required
            ...
            <div>
              <label className="block text-sm font-medium text-[var(--text-secondary)] mb-1">Longitud</label>
              <input
                type="number"
                step="any"
                required
        ...
        {tieneFacturacionDiferente && (
          <div className="space-y-4 rounded-xl border border-[var(--border)] bg-[var(--bg-elevated)] p-4">
            ...
            <div>
              <label className="mb-1 block text-sm font-medium text-[var(--text-secondary)]">Razón social *</label>
              <input
                type="text"
                required={tieneFacturacionDiferente}
                placeholder="Nombre legal de la empresa para facturas"
                className="w-full rounded-lg border border-[var(--border)] bg-[var(--bg-base)] p-2.5 outline-none focus:ring-2 focus:ring-blue-500"
            ...
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
              <div>
                ...
                <input
                  ...
                  required={tieneFacturacionDiferente}
                  placeholder="219999990012"
                  className="w-full rounded-lg border border-[var(--border)] bg-[var(--bg-base)] p-2.5 font-mono outline-none"
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full rounded-lg border border-[var(--border)] bg-[var(--bg-base)] p-2.5 outline-none"
            ...
            <div>
              <label className="mb-1 block text-sm font-medium text-[var(--text-secondary)]">Condición IVA</label>
              <select
                className="w-full rounded-lg border border-[var(--border)] bg-[var(--bg-base)] p-2.5 outline-none"
